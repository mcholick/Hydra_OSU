...
public class LevelUpPanel : MonoBehaviour {
      ...
      * CaculateLvl()
      * 
      * figures out how many lvls the person will gain
      * for every lvl the person will gain 5 points
      * person will be lvl'ed up and the xp needed will be removed from the persons xp pool
      */
     void calculateLvl()
     {
          int xpneeded;
          ...
          while(canlvl == true)
          {
               xpneeded = levels[person.level - 1];
               //if person can lvl
               if (person.xp >= xpneeded)
               {
                    //remove xpneeded from person's xp pool
                    person.xp = person.xp - xpneeded;
                    //give 5 points
                    points = points + 5;
                    //person is lvled up
                    person.level++;
               ...
               {
                    //person could NOT lvl up
                    canlvl = false;
               }

          }
          //holds the total # of points person can assign
          totalpoints = points;
     }
     ...
     void commitPoints()
     {
          ...
          {
               //must assign all points
               Debug.Log("Assign All Points!");
          }
     }
}
